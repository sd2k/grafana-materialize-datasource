...
COPY --chown=grafana --from=yarn-builder /app/grafana-materialize-datasource/dist ${GF_PATHS_PLUGINS}/bsull-materialize-datasource/dist
COPY --chown=grafana --from=rust-builder /usr/src/backend/target/release/grafana-materialize-datasource ${GF_PATHS_PLUGINS}/bsull-materialize-datasource/dist/gpx_grafana-materialize-datasource
RUN GOARCH=$(echo ${TARGETPLATFORM} | sed 's|/|_|') \
  && mv ${GF_PATHS_PLUGINS}/bsull-materialize-datasource/dist/gpx_grafana-materialize-datasource ${GF_PATHS_PLUGINS}/bsull-materialize-datasource/dist/gpx_grafana-materialize-datasource_${GOARCH}
